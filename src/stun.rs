...
async fn test_socket_addr(
    ...
    return match result {
        Ok(return_addr) => if return_addr.port() == local_socket.local_addr().unwrap().port() {
            debug!("{:<25} -> {:<21} returned a healthy response", hostname, &socket_addr);
            StunSocketTestResult {
                socket: socket_addr,
                result: StunSocketResponse::HealthyResponse { rtt: request_duration },
            }
        } else {
            debug!("{:<25} -> {:<21} returned an invalid mapping: expected={}, actual={}", hostname, &socket_addr, local_socket.local_addr().unwrap(), return_addr);
            StunSocketTestResult {
                socket: socket_addr,
        ...
        Err(err) => {
            if err.to_string() == "Timed out waiting for STUN server reply" {
                debug!("{:<25} -> {:<21} timed out after {:?}", hostname, &socket_addr, deadline);
                StunSocketTestResult {
                    socket: socket_addr,
                    result: StunSocketResponse::Timeout { deadline },
                }
            } else {
                debug!("{:<25} -> {:<21} returned an unexpected error: {:?}", hostname, &socket_addr, err.to_string());
                StunSocketTestResult {
                    socket: socket_addr,
